package com.depromeet.ai.manage;
...
public class ClovaStudioManager implements AIPort {
    ...
    @Override // Clova studio Chat Completions API
    public String getChatCompletions(String userContent) {
        final HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
        headers.add("X-NCP-CLOVASTUDIO-API-KEY", clovaStudioProperties.X_NCP_CLOVASTUDIO_API_KEY());
        headers.add("X-NCP-APIGW-API-KEY", clovaStudioProperties.X_NCP_APIGW_API_KEY());
        ChatCompletionsRequest body =
                ChatCompletionsRequest.of(SYSTEM_CONTENT.getPrompt(), userContent);
